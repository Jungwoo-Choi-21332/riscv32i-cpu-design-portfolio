module Hazard_unit(
    ...
    //forward decode
    always @(*) begin
        if (((Rs1D == RdM) && RegWriteM == 1'b1) && (Rs1D != 1'b0)) begin
            ForwardAD = 2'b10; // Case 1
        end
        else if (((Rs1D == RdW) && RegWriteW == 1'b1) && (Rs1D != 1'b0)) begin
    ...
    always @(*) begin
        if (((Rs2D == RdM) && RegWriteM == 1'b1) && (Rs2D != 1'b0)) begin
            ForwardBD = 2'b10; // Case 1
        end
        else if (((Rs2D == RdW) && RegWriteW == 1'b1) && (Rs2D != 1'b0)) begin
